...
// newTestGuard builds a guard with a stubbed resolver so tests don't hit DNS.
func newTestGuard(allowPrivate bool, resolved map[string][]net.IP) *Guard {
	g := NewGuard(allowPrivate, nil, nil)
...
func TestValidateURL_AllowAndDenyLists(t *testing.T) {
	deny := NewGuard(false, []string{"blocked.example.com"}, nil)
	...
	allow := NewGuard(false, nil, []string{"only.example.com"})
